...
type User struct {
	...
	Username string
	Email    string
	Pwd      string //added this for register() in API
	PwHash   string
}
...
func main() {
	...
	// Routes
	r := mux.NewRouter()
	//This handler MUST be before the "/" handler, otherwishe calls to "/layout" will always go to "/"
...
func registerHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		//Call register method in API file, return status code
		_, status := register(r)
